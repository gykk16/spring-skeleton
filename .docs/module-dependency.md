...
| `domain` | Domain | `io.glory.domain` | Domain models: Holiday, CreateHolidayRequest, UpdateHolidayRequest, HolidayNotFoundException |
| `infrastructure` | Technical | `io.glory.infrastructure` | Cache (Caffeine, Redisson), DB (JPA, QueryDSL), HTTP clients, POI, Slack |
| `common-application` | Application | `io.glory.commonapplication` | Common application services: HolidayService (CRUD for holidays) |
| `common-api-app` | Bootstrap (app) | `io.glory.commonapiapp` | Common API server: Holiday API, HolidayController, HolidayDataInitializer |
