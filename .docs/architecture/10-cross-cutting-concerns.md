...
```kotlin
@Pointcut("execution(* io.glory..api..*Controller.*(..))")
fun controllerPointcut()

@Pointcut("execution(* io.glory..facade..*Facade.*(..))")
fun facadePointcut()

@Pointcut("execution(* io.glory..application..*Application.*(..))")
...
logging:
  level:
    io.glory.commonweb.filters: DEBUG
    io.glory.commonweb.interceptors: INFO
```
...
logging:
  level:
    io.glory.commonweb.filters: INFO
    io.glory.commonweb.interceptors: WARN
```
