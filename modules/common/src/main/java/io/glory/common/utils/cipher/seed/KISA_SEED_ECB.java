...
public class KISA_SEED_ECB {
    ...
    public static void KISA_SEED_Decrypt_Block_forECB(int[] in, int in_offset, int[] out, int out_offset,
            KISA_SEED_KEY ks) {
    ...
    public static void SeedRoundKey(int[] pdwRoundKey /* [out] round keys for encryption or decryption */,
        ...
        T[0] = ABCD[ABCD_A] + ABCD[ABCD_C] - KC15;
        T[1] = ABCD[ABCD_B] - ABCD[ABCD_D] + KC15;
        K[30] = SS0[GetB0(T[0]) & 0x0ff] ^ SS1[GetB1(T[0]) & 0x0ff] ^ SS2[GetB2(T[0]) & 0x0ff] ^ SS3[GetB3(T[0])
                & 0x0ff]; // K_16,0
        K[31] = SS0[GetB0(T[1]) & 0x0ff] ^ SS1[GetB1(T[1]) & 0x0ff] ^ SS2[GetB2(T[1]) & 0x0ff] ^ SS3[GetB3(T[1])
                & 0x0ff]; // K_16,1
    }
    ...
    private static void KISA_SEED_Encrypt_Block_forECB(int[] in, int in_offset, int[] out, int out_offset,
            KISA_SEED_KEY ks) {
        int LR[] = new int[4];        // Iuput/output values at each rounds
        int T[] = new int[2];        // Temporary variables for round function F
        int K[] = ks.key_data;        // Pointer of round keys
